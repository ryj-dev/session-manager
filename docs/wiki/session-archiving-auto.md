...
1. **Hook status is idle.** Mid-turn (`working`) or waiting on a permission prompt never archives. A session that has produced no hook traffic at all never archives.
...
4. **No pending background work.** Turn-ending wakeup tools (ScheduleWakeup, Monitor, RemoteTrigger, Workflow, TaskCreate, backgrounded Agent/Task/Bash) leave nothing visible running, but the runtime will re-invoke the session later. The hook server flags these from PostToolUse and the flag blocks archiving until the next user prompt clears it.
...
Archived sessions are merged into the on-quit session save, so quitting with archived sessions still offers them in the next launch's restore prompt.
...
- **A foreground `Agent` tool call mid-conversation** can set the pending-background-work flag until your next prompt (background is the Agent tool's default, and the hook can't always tell); this only delays archiving, never breaks anything.
