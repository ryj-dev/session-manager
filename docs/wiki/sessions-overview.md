...
The registry is also where the observer's digest queue learns that a session ended — its `forget` is the one choke point every teardown path goes through (close, kill, PTY exit, archive). Some kinds are held back at the source: a **drawer preview** is the app re-rendering a conversation, not the user starting one; the **observer's own runs** would digest themselves; and **pipeline/scheduled** sessions are automation whose intent is already known. None of them enter the digest queue (see `observer-agent`).
