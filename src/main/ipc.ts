...
export function registerIpcHandlers(opts: { reinstallMcp: () => void }): void {
  ...
  /** Strip the managed block from CLAUDE.md content; user content untouched. */
  ...
  ipcMain.handle('claude:installClaudeMdInstructions', () => {
    try {
      ...
      // Replace, never no-op: an existing block may be from an older app
      // version, and only the markers delimit what we own — regenerating is
      // the refresh path when the template changes.
      const alreadyInstalled = existing.includes(CLAUDE_MD_MARKER)
      if (alreadyInstalled) {
        const fresh = generateClaudeMdInstructions()
        // Skip the write when the installed block is already current.
        if (existing.includes(fresh.trim())) return { ok: true, alreadyInstalled: true }
        const stripped = stripManagedBlock(existing)
        const separator = stripped.length > 0 && !stripped.endsWith('\n\n') ? '\n\n' : ''
        writeFileSync(claudeMdPath, stripped + separator + fresh, 'utf-8')
  ...
  ipcMain.handle('claude:removeClaudeMdInstructions', () => {
    try {
      const content = readFileSync(claudeMdPath, 'utf-8')
      if (!content.includes(CLAUDE_MD_MARKER)) return { ok: true }
      writeFileSync(claudeMdPath, stripManagedBlock(content), 'utf-8')
      return { ok: true }
    } catch (err) {
      return { ok: false, error: String(err) }
    }
  ...
  // Startup refresh: if the managed block is installed but was generated by
  // an older app version, regenerate it in place. Never installs from
  // scratch — a user who removed the block keeps it removed.
  try {
    const existing = readFileSync(claudeMdPath, 'utf-8')
    if (
      existing.includes(CLAUDE_MD_MARKER) &&
      !existing.includes(generateClaudeMdInstructions().trim())
    ) {
      const stripped = stripManagedBlock(existing)
      const separator = stripped.length > 0 && !stripped.endsWith('\n\n') ? '\n\n' : ''
      writeFileSync(claudeMdPath, stripped + separator + generateClaudeMdInstructions(), 'utf-8')
      console.log('[claude-md] refreshed managed instructions block')
    }
  } catch {
    /* no CLAUDE.md — nothing to refresh */
  }
