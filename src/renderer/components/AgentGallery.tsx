...
interface AgentGalleryProps {
  visible: boolean
  items: PanelItem[]
  onSpawn: (name: string, content: string, allowedTools?: string[]) => void
  onClose: () => void
}

export function AgentGallery({ visible, items, onSpawn, onClose }: AgentGalleryProps): JSX.Element {
        ...
        >
          ...
          <div className="h-10 flex items-center px-4 border-b border-zinc-800/50 shrink-0 titlebar-drag">
            {selectedId ? (
              ...
              <>
                <span className="titlebar-no-drag text-xs text-zinc-300 font-medium">Agents</span>
                <span className="titlebar-no-drag text-[10px] text-zinc-600 ml-2">{items.length} agents</span>
              </>
            )}
          ...
          {selectedId && selectedItem ? (
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="max-w-2xl mx-auto">
                ...
                <button
                  onClick={() => onSpawn(selectedItem.name, selectedItem.content, selectedItem.allowedTools)}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                ...
                <button
                  onClick={() => onSpawn('agent-creator', 'Create a new agent prompt template. Ask me what kind of agent I want to create, then generate a well-structured markdown prompt for it.')}
              ...
              {items.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-sm text-zinc-600 mb-2">No agents yet</p>
                  <p className="text-xs text-zinc-700">Add .md files to resources/agents/</p>
                </div>
              )}
            </div>
          )}
